...
public class AnimationView extends View {
	...
	enum AnimatorStatus {
		PULL_DOWN, DRAG_DOWN, REL_DRAG, SPRING_UP, // rebound to up, the
	...
	private void initView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		mBackPaint.setColor(0xffFf00af);
	...
	protected void onDraw(Canvas canvas) {
		...
		if (mAniStatus == AnimatorStatus.REL_DRAG) {
			...
			int height;
			// NOTICE: If the height equals mLastHeight, then the
			// requestLayout() will not work correctly
			do {
				height = getRelHeight();
	...
	private void drawPopBall(Canvas canvas) {
		...
		mPath.moveTo(0, 0);
		mPath.lineTo(0, PULL_HEIGHT);
		mPath.quadTo(mWidth / 2, PULL_HEIGHT - PULL_DELTA, mWidth, PULL_HEIGHT);
		mPath.lineTo(mWidth, 0);
		canvas.drawPath(mPath, mBackPaint);
	...
	private void drawDone(Canvas canvas) {
		...
		if (getDoneRatio() < 0.3) {
			...
			int afterColor = Color.argb((int) (0xff * (1 - getDoneRatio() / 0.3f)), Color.red(beforeColor), Color.green(beforeColor),
